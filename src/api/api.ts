...
import { BankGoldTransactionSchema, BankItemTransactionSchema, CharacterFightDataSchema, CharacterMovementDataSchema, CharacterRestDataSchema, CharacterSchema, EquipRequestSchema, GameError, RewardDataSchema, SkillDataSchema, Slot, TaskDataSchema, TaskFullSchema } from "./types.js";
...
export default {
  ...
  moveCharacter: async (characterName: string, x: number, y: number): Promise<ApiResponse<CharacterMovementDataSchema>> => {
    return await doPost(`/my/${characterName}/action/move`, {
      x: x,
      y: y
    })
  },
  fight: async (characterName: string): Promise<ApiResponse<CharacterFightDataSchema>> => {
    return await doPost(`/my/${characterName}/action/fight`)
  },
  rest: async (characterName: string): Promise<ApiResponse<CharacterRestDataSchema>> => {
  ...
  craft: async (name: string, code: string, quantity: number): Promise<ApiResponse<SkillDataSchema>> => {
    return await doPost(`/my/${name}/action/crafting`, { code, quantity })
  },
  equip: async (name: string, code: string, slot: Slot, quantity: number = 1): Promise<ApiResponse<EquipRequestSchema>> => {
    return await doPost(`my/${name}/action/equip`, { code, slot, quantity })
  },
  unequip: async (name: string, slot: Slot, quantity: number = 1): Promise<ApiResponse<EquipRequestSchema>> => {
  ...
  withdrawBank: async (name: string, code: string, quantity: number): Promise<ApiResponse<BankItemTransactionSchema>> => {
    return await doPost(`/my/${name}/action/bank/withdraw`, { code, quantity })
  },
  depositBankGold: async (name: string, quantity: number): Promise<ApiResponse<BankGoldTransactionSchema>> => {
    return await doPost(`/my/${name}/action/bank/deposit/gold`, { quantity })
  },
  withdrawBankGold: async (name: string, quantity: number): Promise<ApiResponse<BankGoldTransactionSchema>> => {
...
const doPost = async <T>(path: string, data: object = {}): Promise<ApiResponse<T>> => {
  try {
